...
struct TaskQueue
{
    ...
    void wait()
    {
        ...
        if (m_sleep) {
            m_sleep_condition.wait(lock, [this] { return !m_sleep; });
        } else {
...
struct Worker
{
    ...
    Worker(TaskQueue& queue, uint64_t const id)
        : m_id{id}
        , m_queue{&queue}
    {
...
struct ThreadPool final
{
    ...
    {
        dispatch(container.size(),[&callback](size_t const start, size_t const end) {
